...
| カード名 | タイミング | 効果概要 | 実装状況 | 備考 |
| --- | --- | --- | --- | --- |
| アイゼン | バトル時 | BP3000以下の相手スピリット1体破壊 / 自軍アルティメット時1ドロー | 実装済み | アタック時・ブロック時の両方に接続。候補1件は即時解決、複数は対象選択 |
| ファイザード | なし | 効果なし | 実装済み | 効果なしカードとして扱う |
| ナイト | 召喚時 / アタック時 | 自軍アルティメット時に相手ネクサス破壊 / アタック時1ドロー | 実装済み | ネクサス候補1件は即時解決、複数は対象選択 |
| リューマン | 破壊時 | BP4000以下の相手スピリット1体破壊 | 実装済み | BP比較・マジック破壊・効果破壊・0コア破壊の全経路で共通破壊処理を通す |
| ウルフ | アタック時 | BP+3000 / 自軍アルティメット時さらに+3000 | 実装済み | battleOnlyBpBonus で管理し解決後に消去 |
| ARジークF | 召喚条件 / アタック時 / フラッシュ | 赤スピリット1体条件 / デッキ破棄と強制ブロック / コア移動でBP+3000 | 実装済み | スピリットでブロックされた時のライフ移動も実装 |
| ARジークV | 召喚条件 / 召喚時 / アタック時 | 赤スピリット3体条件 / デッキ破棄とライフ回復 / 強制ブロックとBP12000以下破壊 | 実装済み | ライフ回復は最大5まで |
| ダブルドロー | メイン / フラッシュ | 2ドロー / 自軍アルティメット時+1ドロー / BP+1000 | 実装済み | フラッシュ時は対象選択 |
| シャイニングフレイム | フラッシュ | BP10000以下の相手スピリット1体破壊 | 実装済み | 候補0件でも停止しない |
| フレイムテンペスト | フラッシュ | BP8000以下1体破壊 or BP4000以下全体破壊 | 実装済み | 単体は対象選択、全体は即時解決 |
| 狩る者の集落 | 継続 / ライフ減少時 | 自軍赤スピリット・アルティメットBP+2000 / BP4000 or 5000以下破壊 | 実装済み | 自軍アルティメットがいる時は破壊上限5000 |

### Current Notes

- 効果処理本体は現在も主に `server/src/game.ts` にあります。
- 効果一覧のメタ情報は `server/src/effects.ts` に分離しました。
- 未実装カード効果は現時点ではありません。挙動を簡略化しているのは、候補1件時に即時解決する対象選択効果がある点です。
