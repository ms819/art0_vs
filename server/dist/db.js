...
export async function getAllCards(db) {
    const rows = await db.all("SELECT * FROM cards ORDER BY id ASC");
    return rows.map(mapRowToCard);
}
export async function loadSourceCards() {
...
function mapRowToCard(row) {
    return {
        ...
        name: row.name,
        cost: row.cost,
        reduction: JSON.parse(row.reduction),
        color: row.color,
...
function normalizeType(cardId, type) {
    if (type === "arutimetto") {
        return "ultimate";
    }
    if (cardId === "A-007") {
...
async function shouldRecreateCardsTable(db) {
