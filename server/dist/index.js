...
async function main() {
    ...
    function broadcastState(room) {
        ...
        room.players.forEach((player, index) => {
            ...
            const payload = {
                type: "STATE_UPDATE",
                state: toClientState(room.gameState, index),
            };
            log("broadcast state", {
                roomId: room.id,
                playerIndex: index,
    ...
    function sendRoomStatus(room, playerIndex) {
        const player = room.players[playerIndex];
        if (!player?.socket || player.socket.readyState !== WebSocket.OPEN) {
            return;
        }
    ...
    function broadcastRoomStatus(room) {
        room.players.forEach((_player, playerIndex) => sendRoomStatus(room, playerIndex));
    }
}
